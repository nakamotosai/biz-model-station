#!/usr/bin/env python3
"""Patch generate_site.py: shimmer fix + mobile related items + true lazy loading"""
import json, re
...
src = src.replace(".color-ribbon{{display:flex;height:6px;border-bottom:3px solid var(--stroke)}}",
                  ".color-ribbon{{display:flex;height:10px;border-bottom:3px solid var(--stroke)}}")
src = src.replace("color:rgba(255,255,255,.35) 50%", "color:rgba(255,255,255,.55) 50%")
...
if "related-grid{grid-template-columns:1fr;gap:6px}" not in src:
    # Insert after sticky-bar.hide CSS (detail page version)
    src = src.replace(
    ...
    print("2. Mobile CSS SKIP (exists)")
