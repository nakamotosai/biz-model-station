...
# 2026-08-09 修正（P1-13）：与 generate_site 同源归一（此前 md 写「中国/食品饮料」、
# site 写「中/其他」漂移）。NORM/norm_industry/IND 目录与 generate_site.py 保持同步。
NORM = {
...
def load_cards() -> tuple[list[dict], list[str]]:
    """读取并归一全部条目，返回 (valid_cards, errors)。坏条目跳过不阻塞（2026-08-09 修，与 generate_site 同策略）。"""
    cards = []
    errors: list[str] = []
    for f in sorted(DATA.glob("*.json")):
        ...
        if missing:
            errors.append(f"[{f.name}] 缺必填字段: {missing}")
            continue
        cards.append(m)
    return cards, errors
...
def main() -> None:
    ...
    if errors:
        print(f"[warn] {len(errors)} 条数据缺失/损坏已跳过:")
        for e in errors:
            print(f"      {e}")
